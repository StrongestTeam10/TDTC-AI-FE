...
### 2026-08-11 (10차 - 시장 오브젝트 편집 화면: 출입구 흡수 + 오브젝트 배치 + 통로 정책)
- **요청**: 시장 구조 등록에 "시장 오브젝트" 대상 추가(A안 - 출입구를 흡수). "컬럼이
  같아야 하나?"는 질문에 → 아니오, 화면만 묶고 종류별로 저장소가 다름을 설명
- ➕ `components/MarketObjectEditor.tsx` 신규: "시장 오브젝트" 대상 전용 편집기.
  하위 종류를 드롭다운으로 고르며, **종류마다 폼·목록·저장소가 다름**:
  - **출입구(GATE)** → 시설 API(mrkfcts01m). 지도 클릭 위치 + 개방/폐쇄. 마커 클릭 수정
  - **푸드트럭/행사존/휴게공간/장애물** → `mrkobjt01m.objects`. 지도 클릭 →
    좌표 + **소속 구역 자동 판정**(point-in-polygon) + **강도(intensity)** 슬라이더
  - **통로 제어 정책** → `mrkobjt01m.corridorPolicies`. 출발/도착 구역 드롭다운 + 동작(폐쇄/개방/일방)
  - 오브젝트/정책은 시장당 1세트라 전체를 로드해 편집 후 통째 저장(PUT)
- ✏️ `pages/FacilityManagePage.tsx`: 대상 선택기에서 "출입구" 제거하고 "시장 오브젝트"
  추가. `MARKET_OBJECT` 대상이면 **early return**으로 `MarketObjectEditor`를 렌더(기존
  상점/CCTV 화면 로직은 그대로 둠). 시장 탭/대상 선택기를 공통 변수로 추출해 재사용
  - `TargetKind`에 'GATE'는 남겨둠 - 상점 폼이 GATE와 마크업을 공유해 `=== 'GATE'`
    비교가 여러 곳인데 지금은 GATE를 못 골라 도달하지 않을 뿐(상점 전용 정리는 UIUX 단계)
- 검증: `npx tsc -b`/`npx oxlint`/`npx vite build` 통과
- ⚠️ **BE 재기동으로 schema-init.sql을 태워 `mrkobjt01m`을 생성**해야 오브젝트/정책 저장
  이 동작함(출입구는 기존 시설 테이블이라 무관)
- 남은 연결: 시뮬레이션 비교가 이 초기 배치를 before/after 초기값으로 불러오는 부분
  (사용자가 그쪽 화면에서 이어서 쓸 예정)

### 2026-08-11 (9차 - 헤더 개편 + 시장 오브젝트 저장 API 연결, 화면은 다음 단계)
- **요청**: 헤더 "상점 위치 등록" → "시장 구조 등록", 메뉴 순서 재배치. 시장 구조
  등록에 시뮬레이션 초기 배치용 오브젝트/통로 정책 데이터 기능 추가(출입구는 시장
  오브젝트로 흡수). 규모가 커서 이번 턴은 헤더 + 데이터 계층까지, 화면은 다음 단계
- ✏️ `components/layout/Header.tsx`: 메뉴 순서를 **시뮬레이션 비교 > 시나리오 이력 >
  관제 대시보드 > 시장 구조 등록 > 회원관리 > 게시판**으로. "상점 위치 등록" →
  "시장 구조 등록"(경로 /facilities 유지)
- ✏️ `pages/FacilityManagePage.tsx`: 화면 상단 H1도 "시장 구조 등록"으로
- ✏️ `api/client.ts`: `MarketObjectConfig` 타입 + `fetchMarketObjects`/`saveMarketObjects`.
  objects/corridorPolicies는 시뮬레이션 요청과 같은 `PlacedObject`/`CorridorPolicy` 형식
- 확정된 설계(재재님): 오브젝트 배치 + 통로 정책을 새 JSON 테이블(mrkobjt01m)에 시장당
  1세트로 저장 → 시뮬레이션 비교가 before/after 초기값으로 **둘 다 동일하게** 불러오고
  after만 수정. 실제 실행 데이터는 기존 baseline/scenario 테이블에. 출입구는 시장
  오브젝트 대상으로 흡수. BE 상세는 `TDTC-AI-BE/CHANGELOG.md` 2026-08-11(3차)
- **다음 단계(화면)**: 시장 구조 등록의 대상 선택기에서 "출입구"를 "시장 오브젝트"로
  바꾸고, 오브젝트 배치(지도 클릭 → 종류/강도) + 통로 정책(구역 쌍 → 폐쇄) 편집 UI 구현
- 검증: `npx tsc -b`/`npx oxlint` 통과

### 2026-08-11 (8차 - CCTV 꼭짓점을 클릭한 위치 그대로 찍도록: 위치 왜곡 제거)
- **요청**: 체크한 4곳을 순서대로 눌렀는데 폴리곤 좌표가 엉뚱하게 나옴. 캡처처럼
  1·2·3·4가 클릭한 자리에 그대로 위치하게 해달라
- **원인**: 7차에서 넣은 두 보정이 클릭 위치를 옮기고 있었다
  - **방향 보존 스냅**: 밖 클릭을 "구역 중심 방향"으로 당겨, 찍은 위치가 중심 쪽으로 왜곡
  - **선분 제약(clampVertexIntoPolygonWithEdge)**: 잇는 선이 구역을 벗어나면 점을 경계
    앞으로 당김 → 1번이 좁은 끝 꼭짓점 근처면 2번이 1번 쪽으로 끌려감
  두 보정이 겹쳐, 가장자리를 따라 찍어 큰 사각형을 만들려는데 점들이 안쪽으로 뭉쳤다
- ✏️ `utils/polygonGeometry.ts` `snapPointIntoPolygon`: **최단거리 방식으로 되돌림**.
  밖 클릭은 "가장 가까운 경계 안쪽 지점"으로만 최소 이동(안쪽 클릭은 왜곡 0). 클릭
  위치를 최대한 보존한다
- ✏️ `pages/FacilityManagePage.tsx` `handleAddVertex`: **선분 제약과 닫는 변 검사 제거**.
  클릭 순서대로 4점을 그대로 사각형 꼭짓점으로 삼는다. 점은 반드시 구역 안(스냅)이지만,
  점을 잇는 선이 구역을 벗어나는지는 강제하지 않는다 - 강제하면 찍은 위치가 옮겨져
  원하는 사각형이 안 나오기 때문. (각 꼭짓점이 구역 안인지는 BE도 point-in-polygon으로
  재검증하므로 소속 조건은 그대로 지켜진다)
- `isSegmentInsidePolygon`/`clampVertexIntoPolygonWithEdge`는 미사용으로 남겨둠(선분
  제약이 다시 필요하면 사용). 주석으로 표시
- 검증: 실제 북측 구역 좌표로 가장자리 4클릭 → 4점이 서로 다르고 각자 클릭 위치대로
  (안쪽 0m, 밖 8~12m 최소 이동) 찍힘. `npx tsc -b`/`npx oxlint` 통과. FE만 변경
- ⚠️ 트레이드오프: 오목하거나 매우 좁은 구역에서는 4점을 잇는 선이 구역을 살짝
  벗어날 수 있음(점은 항상 구역 안). 위치 보존을 우선한 결정

### 2026-08-11 (7차 - 가늘고 긴 구역에서 CCTV 꼭짓점이 한 점으로 뭉치던 버그)
- **요청**: 1번 점을 찍고 북측 아래를 멀리 클릭하면 2번이 1번과 같은 위치로 찍힘.
  사각형을 만드는 로직이어야 하는데 점이 뭉침
- **원인**: 북측 구역은 도로를 따라 **가늘고 긴 띠** 모양인데, `snapPointIntoPolygon`이
  "가장 가까운 경계점"으로 스냅해서 밖의 넓은 영역이 전부 같은 짧은 변/꼭짓점으로
  수렴했다. 게다가 1번이 폴리곤 좁은 끝의 꼭짓점 근처면, 다음 점으로 가는 어떤 선분도
  즉시 폴리곤을 나가 `clampVertexIntoPolygonWithEdge`가 **1번 자리로 당겨** 붕괴시켰다
  (실제 북측 구역 좌표로 재현 확인)
- ✏️ `utils/polygonGeometry.ts`:
  - `snapPointIntoPolygon`을 **방향 보존 스냅**으로 교체: "가장 가까운 경계점"이 아니라
    **구역 중심에서 클릭 방향으로 광선을 쏴** 경계와 만나는 지점 살짝 안쪽으로 스냅.
    남쪽을 클릭하면 남쪽 경계로, 동쪽이면 동쪽 경계로 가 **서로 다른 클릭이 서로 다른
    안쪽 점**이 됨. 스냅점들이 중심 근처에 모여 잇는 선분도 대체로 구역 안에 들어옴.
    (오목해서 중심이 폴리곤 밖이면 예전 최단거리 방식으로 폴백)
  - `clampVertexIntoPolygonWithEdge`에 **뭉침 방지** 추가: 선 제약으로 당긴 점이
    이전 점과 거의 겹치면(구역 대각선의 3% 미만) 선이 살짝 삐져나가더라도 방향 보존
    스냅점을 그대로 써서 사각형이 형성되게 함(가늘고 긴 구역 대비)
- 검증: 실제 북측 구역 좌표로 1번(꼭짓점 근처)+밖 3클릭 → **4점이 모두 서로 다르게**
  찍히고 각 점이 구역 안. 넓은 볼록 사각형 회귀도 정상. `npx tsc -b`/`npx oxlint` 통과
- FE만 변경, BE·DB 무관

### 2026-08-11 (6차 - 마커 정보창 누적 버그 + 클릭 시 좌표 변경 버그 + 검색 버튼화)
- **요청**: (1) 마커를 여러 개 클릭하면 이전 정보창들이 안 닫히고 화면에 쌓임
  (스크린샷 확인) (2) 빨간 마커 표시는 좋은데 마커를 선택할 때마다 좌표가 계속
  바뀌는 걸 없애달라 (3) 상점 검색을 타이핑마다 바로 거르지 말고 검색 버튼으로
- **원인 진단**:
  - **정보창 누적**: `InfoWindow`는 `Marker`와 별개 객체라 `marker.setMap(null)`로는
    안 닫힘. 마커 클릭 → 수정 진입 → `mapMarkers`가 편집 중인 시설을 필터링 →
    마커 목록 이펙트 재실행 → 마커는 지워지지만 그 사이 호버로 열려 있던 정보창은
    닫힐 방법이 없어 지도 위에 계속 쌓였다(스크린샷의 여러 이름표가 이 잔재)
  - **클릭 시 좌표 변경**: 카카오맵은 마커 클릭 이벤트가 **기본적으로 지도 클릭
    이벤트로도 전파**된다. `preventMap()`을 안 불러서, 마커를 클릭해 정확한 시설
    좌표로 수정 진입한 직후 지도의 click 리스너(`onPick`)가 또 실행되며 클릭한
    화면 픽셀의 대략적 좌표로 덮어썼다. 저장 시 좌표가 미세하게 밀릴 수 있는
    실제 버그였음
- ✏️ `components/FacilityLocationPicker.tsx`:
  - `infoWindowLayerRef`로 정보창을 추적해, 마커를 다시 그릴 때마다 이전 정보창을
    전부 닫음. `openInfoWindowRef`로 "지금 열린 정보창 1개"만 유지해 다른 마커에
    올리면 이전 것부터 닫음(동시에 여러 개가 뜨는 것 자체를 막음)
  - 마커 클릭 핸들러에 `kakao.maps.event.preventMap()` 추가 - 지도 클릭으로의
    전파를 막아 `onPick`이 덮어쓰지 않게 함. 정보창도 클릭 시 함께 닫음
- ✏️ `pages/FacilityManagePage.tsx`: 상점 검색을 `stallSearchInput`(입력 중)과
  `stallSearch`(적용값)로 분리. 검색 버튼 클릭 또는 Enter를 눌러야 목록에 반영됨
  (타이핑마다 즉시 필터링하던 것 제거). 대상(targetKind) 전환 시 검색어도 초기화
- 검증: `npx tsc -b`/`npx oxlint`/`npx vite build` 통과. FE만 변경, BE·DB 무관

### 2026-08-11 (5차 - 선 걸림도 안쪽 스냅 + 구역 이동 버튼 + 마커 개선 + 상점 검색)
- **요청**: (1) 점 잇는 선이 걸릴 때도 안쪽 점을 찍어달라 (2) 선택 구역으로 이동하는
  버튼 (3) 출입구/상점 마커를 더 작고 안 겹치게, 마커 클릭 시 수정과 동일 동작
  (4) 상점 목록에 이름 검색
- ➕ `utils/polygonGeometry.ts` `clampVertexIntoPolygonWithEdge`: 밖 클릭은 안쪽으로
  스냅하고, 직전 점과 잇는 선이 오목 구역 밖으로 새면 경계를 처음 만나는 지점 바로
  앞(안쪽)까지 당겨 찍음("선은 구역 안에서 멈춘다"). 선분-경계 교차 지점 t를 구해 처리.
  ㄷ자 오목 폴리곤으로 홈을 가로지르는 클릭이 경계 앞에서 멈추는지 스크립트 확인
- ✏️ `pages/FacilityManagePage.tsx` `handleAddVertex`: 점 스냅 + 선 클램프를 합친
  위 함수로 교체(직전 변은 자동 보정, 4번째 닫는 변만 자동 보정이 어려워 안내로 남김)
- ✏️ `components/FacilityLocationPicker.tsx`:
  - **작은 원형 마커**(16px SVG 점)로 교체 - 기본 큰 핀이 밀집 시 겹치던 걸 완화하고
    개별 선택이 쉬움. active 여부로 색 구분(파랑/회색), 호버 시 이름 말풍선
  - **마커 클릭 → `onMarkerClick`**(수정 진입). CCTV 대상에선 비활성
  - `refitToken` prop: 값이 바뀌면 지도를 소속 구역에 다시 맞춤("선택 구역으로 이동")
- ✏️ `pages/FacilityManagePage.tsx`:
  - 지도 위에 **"🎯 선택 구역으로 이동" 버튼**(CCTV 대상, 구역 선택 시). 패닝해서
    구역을 벗어났을 때 되돌아옴
  - 마커 클릭 → 해당 시설 `startEditFacility`(수정 버튼과 동일)
  - **상점 목록 이름 검색**: 상점 대상에서만 검색창 노출. 지도 마커는 전체 유지하고
    하단 목록만 이름으로 필터(클라이언트). 검색어 변경 시 첫 페이지로, 결과 없음 안내
- 검증: `npx tsc -b`/`npx oxlint`/`npx vite build` 통과. 기하는 볼록/오목 케이스 스크립트 확인
- FE만 변경, BE·DB 무관(재기동 불필요)

### 2026-08-11 (4차 - CCTV 좌표: 밖 클릭 스냅 + 지도 확대/자동 맞춤)
- **요청**: (1) 폴리곤 밖을 찍으면 "선택 불가"로 막지 말고 최단거리 내부 점으로 대신
  찍어달라 (2) 카카오 맵을 두 칸 더 확대할 수 있게(폴리곤 좌표가 잘 안 나옴)
- ➕ `utils/polygonGeometry.ts` `snapPointIntoPolygon`: 폴리곤 밖 점을 경계에서 가장
  가까운 안쪽 점으로 끌어당김. 각 변에 수선의 발(선분 범위 클램프)을 구해 최단을 고르고,
  경계 위 부동소수 오차를 피하려 중심 쪽으로 EPS(≈1cm)만큼 밀어 확실히 안쪽에 놓음
- ✏️ `pages/FacilityManagePage.tsx` `handleAddVertex`: 밖 클릭을 거부하는 대신
  `snapPointIntoPolygon`으로 스냅해 찍음. 스냅 후에도 변이 오목 구역 밖으로 새는
  경우만 거부(스냅으로 못 고치는 케이스)
- ✏️ `components/FacilityLocationPicker.tsx`:
  - 초기 확대 레벨 3 → 2, `setMinLevel(1)`로 스크롤 최대 확대(레벨 1)까지 허용.
    ⚠️ 카카오 로드맵은 레벨 1이 확대 하한이라 그 이상은 API가 지원하지 않음
  - CCTV 모드에서 소속 구역을 고르면 `map.setBounds()`로 그 구역이 화면을 꽉 채우게
    자동 맞춤 → 시장 중심에서 떨어진 구역도 바로 보이고 최대한 크게 확대돼 좌표를
    정밀하게 찍을 수 있음(소속 구역이 바뀔 때만 실행, 매 클릭엔 안 튐)
- 검증: `npx tsc -b`/`npx oxlint` 통과. 스냅은 실제 망원시장 구역 유사 좌표로 위/좌/우/
  원거리 밖 점이 모두 안쪽으로 들어오는지 스크립트 확인

### 2026-08-11 (3차 - CCTV 구역 재설계: 폴리곤 제약 + 단일항목/페이징 + 게이트 개방/폐쇄)
- **요청**: (1) CCTV 4점과 잇는 선이 소속 구역 밖으로 못 나가게 (2) 하단 목록에
  게시판식 페이징 (3) CCTV를 구역 1~4 고정이 아니라 등록마다 추가되는 단일 항목으로,
  zone 테이블 조인해 세팅(zone_id 기준) (4) CCTV 사용/미사용, 출입구 개방/폐쇄
- ➕ `utils/polygonGeometry.ts`: point-in-polygon(ray casting) + 선분 교차 + 선분이
  폴리곤 안에 완전히 들어있는지 검사. **오목한 구역**에서 두 점은 안이어도 잇는 선이
  홈으로 삐져나오는 경우를 교차 검사로 잡음(테스트로 확인). BE와 같은 알고리즘
- ✏️ `components/FacilityLocationPicker.tsx`: `constraintPolygon` prop 추가. 소속 구역을
  파란 영역으로 강조해 "여기 안에만 찍을 수 있다"를 보여줌. 실제 밖 클릭 거부는 부모가
  검증 후 처리(거부된 점은 vertices에 안 들어가 화면에 안 그려짐)
- ✏️ `pages/FacilityManagePage.tsx` 대폭 재구성:
  - 등록 대상: 출입구 / 상점 / **CCTV 구역(단일)**. CCTV는 "소속 시뮬레이션 구역"
    드롭다운으로 zone_id를 고르면 그 폴리곤이 제약이 됨
  - 지도 클릭마다 point-in-polygon + 직전 변 + 닫는 변(4→1)을 검사해 밖으로 나가면
    거부하고 사유 표시. 구역을 바꾸면 이전 기준으로 찍은 점은 비움
  - **추가(POST)**마다 목록에 쌓임 / 수정(PUT)/삭제(DELETE) by id
  - CCTV 목록: 소속 구역명(zone 조인) / 꼭짓점 수 / 사용·미사용 / 비고. **서버 페이징**
  - 출입구: 개방/폐쇄 토글(is_active), 목록 상태도 개방/폐쇄로 표기
  - 하단 목록에 게시판식 번호 페이저 추가. **CCTV는 서버 페이징**, 시설(출입구/상점)은
    지도가 전체 마커를 필요로 해 전량을 받아두고 **목록만 클라이언트에서 잘라** 페이징
- ✏️ `api/client.ts`: `CctvZone`에 zoneId/isActive, zoneNo/별도 zoneName 입력 제거
  (zoneName은 서버가 조인해 내려줌). `fetchCctvZones(marketId, page, size)` → PageResponse,
  `createCctvZone`(POST)/`updateCctvZone`(PUT id)/`deleteCctvZone`(id)
- 검증: `npx tsc -b`/`npx oxlint`/`npx vite build` 통과. 기하 로직은 볼록/오목 케이스
  스크립트로 확인. BE 상세는 `TDTC-AI-BE/CHANGELOG.md` 2026-08-11(2차) 참고
- ⚠️ **BE 재기동으로 schema-init.sql을 먼저 태워야** CCTV 저장이 동작(zone_id/is_active
  컬럼 필요). 1차로 넣어둔 CCTV 구역 행은 zone_id가 없어 새로 등록해야 함

### 2026-08-11 (2차 - CCTV 구역 그릴 때 시뮬레이션 구역을 참고로 표시)
- **요청**: CCTV 구역을 설정할 때 시뮬레이션 비교 화면과 같은 로직으로 그 시장의
  시뮬레이션 구역을 지도에 찍어달라
- ✏️ `pages/FacilityManagePage.tsx`: `loadData`에 `fetchZones`(시뮬레이션 구역 조회)를
  추가. 참고용 배경일 뿐이라 실패해도 오류 배너를 띄우지 않음(관제요원이 아니면
  `/simulation/**`이 403이라 조용히 비워두는 게 맞음). CCTV 구역 대상일 때만, 그리고
  "시뮬레이션 구역 참고로 표시" 체크박스가 켜져 있을 때만 오버레이로 넘김
- ✏️ `components/FacilityLocationPicker.tsx`: `simulationZones` prop 추가. 시뮬레이션
  구역은 **회색 점선**(`strokeStyle: 'shortdash'`, KakaoMapView 통로 표시에 쓰던 값)으로
  CCTV 구역(실선)과 구분되게 그리고, 중심점에 구역 이름 라벨을 얹음. CCTV 구역보다
  먼저 깔아 CCTV 구역이 위에 오게 함. 클릭에는 관여하지 않는 배경 레이어
- 시뮬레이션 구역(mrkaddr01d)과 CCTV 구역(mrkcctv01m)은 좌표 형식이 같은 GeoJSON이라
  기존 `geoJsonToVertices`를 그대로 재사용. **읽기 전용 표시일 뿐 저장·수정하지 않음** —
  이 화면에서 시뮬레이션 구역이 바뀌는 일은 없다
- 검증: `npx tsc -b`/`npx oxlint` 통과

### 2026-08-11 (상점 위치 등록 - 등록 대상 선택기 추가 + CCTV 좌표 구역 등록)
- **요청**: 상점 정보 입력 최상단에 콤보박스로 지도 마커와 데이터를 함께 제어.
  구성은 출입구 / 상점 / CCTV 좌표 구역 1~4(JSON 저장)이고, 대상에 따라 입력 폼도 변경
- ✏️ `pages/FacilityManagePage.tsx`: 폼 최상단에 "등록 대상" 셀렉트 추가. 선택에 따라
  지도 마커·입력 폼·하단 목록이 한꺼번에 바뀜
  - **출입구** → `mrkfcts01m`(facility_type='GATE'), 단일 점. 업종·영업중 필드는 숨김
    (출입구엔 해당 개념이 없어 항상 사용중으로 저장). 목록도 출입구만 표시
  - **상점** → `mrkfcts01m`(그 외 타입), 단일 점. 업종 목록에서 '출입구'는 제거
    (대상 선택기에서 이미 갈라지므로 중복)
  - **CCTV 구역 1~4** → `mrkcctv01m`, 사각형 4점. 구역명 / 꼭짓점 4칸(번호별 좌표) /
    저장될 GeoJSON 미리보기 / 비고로 폼이 통째로 바뀜. 마지막 점 취소·모두 지우기 제공
  - 87개 상점 마커에 출입구가 파묻히던 문제도 같이 해소됨(대상별 마커 필터링)
- ✏️ `components/FacilityLocationPicker.tsx`: `mode` prop 추가
  - `'point'`(기존) / `'rectangle'`(클릭 4번으로 꼭짓점). 꼭짓점에 순서 번호를 붙여
    표시 — 찍는 순서가 뒤집히면 폴리곤이 꼬여서 눈으로 확인할 수 있어야 함
  - 저장된 CCTV 구역은 옅은 배경 폴리곤으로 항상 표시(겹침 확인용). 편집 중인 구역은
    빨간 미리보기와 겹치므로 배경에서 제외
  - 지도 클릭 리스너는 최초 1회만 등록하고 최신 mode/콜백은 ref로 넘김(재등록하면 중복 누적)
- ➕ `utils/cctvZonePolygon.ts`: GeoJSON([경도,위도]) ↔ 카카오맵([위도,경도]) 변환.
  GeoJSON 링은 첫 점과 끝 점이 같아야 해서 저장 시 첫 점을 한 번 더 넣고, 읽을 때 제거.
  구역 번호별 고정 색상(`zoneColor`)도 여기 둠
- ✏️ `api/client.ts`: `CctvZone` 타입 + `fetchCctvZones`/`saveCctvZone`/`deleteCctvZone`
- ⚠️ **CCTV 구역은 시뮬레이션 구역과 다른 테이블(`mrkcctv01m`)에 저장됨** — 같은
  `mrkaddr01d`에 넣으면 SIM이 CCTV 구역까지 에이전트 배치 대상으로 잡아 시뮬레이션
  결과 수치가 바뀌고, 시뮬레이션 비교 화면 지도에도 폴리곤이 겹쳐 그려짐. BE 쪽 상세는
  `TDTC-AI-BE/CHANGELOG.md` 2026-08-11 항목 참고
- 검증: `npx tsc -b`/`npx oxlint` 통과. GeoJSON 왕복 변환과 BE의 4점 검증 규칙이 맞는지
  스크립트로 확인(닫힌 링 5점 → BE가 4점으로 계산 → 통과, 3점이면 400)
