...
import type { ScenarioRequest, Zone, PlacedObject, EventTrigger } from '../types';

type PlacementKind = PlacedObject['objectType'] | EventTrigger['eventType'];

interface ScenarioFormProps {
  isRunning: boolean;
  onSubmit: (request: Pick<ScenarioRequest, 'agentCount' | 'steps'>) => void;
...
// 2026-07-25 추가: 화재/음향 이상 - 오브젝트와 같은 팔레트에서 같은 방식(지도 클릭)으로 배치.
...
export default function ScenarioForm({
  ...
  return (
      <form onSubmit={handleSubmit} className="space-y-3 rounded-lg border border-slate-700 bg-slate-800 p-3">
        ...
        {/* 2026-07-25 추가: 이벤트 발생 (화재/음향 이상) - 오브젝트와 동일한 배치 방식 */}
        <div className="rounded-lg border border-red-800 bg-red-950/30 p-2.5 space-y-2">
          ...
          {events.length > 0 && (
              <ul className="space-y-1 max-h-28 overflow-y-auto">
                {events.map((ev, i) => (
                    <li key={i} className="flex items-center justify-between rounded bg-slate-900/60 px-2 py-1 text-[11px] text-slate-300">
                      <span>{eventTypeLabel(ev.eventType)} · 강도 {ev.intensity}</span>
                      <button
                          type="button"
